...
//@Singleton
//class RandomUserPagingSource(private val networkService: NetworkService,
...
//                nextKey = if (deserializedResponse.results.isEmpty()) null else page.plus(1),
//            )
//        } catch (e: Exception) {
//            LoadResult.Error(e)
//        }
...
class RandomUserPagingSource(
    ...
    private val gson: Gson,
    private val totalResults: Int // The input value that determines the total number of results
) : PagingSource<Int, Result>() {
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, Result> {
        return try {
            val page = params.key ?: INITIAL_PAGE
            val totalPages =
                (totalResults + PAGE_SIZE - 1) / PAGE_SIZE  // Calculate total pages (round up)

            // Calculate results for the current page
            val resultsForPage = if (page == totalPages) {
                totalResults % PAGE_SIZE // Remaining results on the last page
            } else {
                PAGE_SIZE // Full page of results (10)
            }
            ...
            // Convert the response to JSON and deserialize using the custom Gson
            val jsonResponse = gson.toJson(response)
            val deserializedResponse = gson.fromJson(jsonResponse, RandomUserResponse::class.java)
            ...
            LoadResult.Page(
                data = deserializedResponse.results,
                prevKey = if (page == INITIAL_PAGE) null else page.minus(1),
                nextKey = if (page >= totalPages) null else page.plus(1)  // Only load the next page if not at the last page
            )
        } catch (e: Exception) {
            LoadResult.Error(e)
        }
